...
DOMAIN_SEARCH_NOTES = (
    ...
    "plus a specific staff-recommended tire from the catalog (not just the computed range), "
    "even before a tire is chosen — lead with that pick rather than making the customer ask "
    "which tire to get. When the ideal match happens to be out of stock, say so and name the "
    "in-stock pick's trade-off rather than only naming whichever one is available. Called on "
    "either a wheel or a tire that has the other already in the cart, it states whether they "
    "actually fit together — a tire's diameter "
